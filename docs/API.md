...
| Scenes | `routes/api.php`, 6 | `GET /conversation/scenes`, `GET /conversation/scenes/{scene}`, `POST /conversation/scenes/start`, `GET /conversation/scene-sessions/{session}`, `POST /conversation/scene-sessions/{session}/beats/{beat}/answer`, `POST /conversation/scene-sessions/{session}/finish`. Conversation practice with the room drawn around it — see §5.9. Starting a run also answers with `player_url`: a signed, two-hour link to the page that draws the scene. No model call per line, so these sit on the ordinary limiter rather than the `ai` one. |
